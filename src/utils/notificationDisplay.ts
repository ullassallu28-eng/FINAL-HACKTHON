...
export function getVetDecisionStatus(notification: NotificationItem): VetDecisionStatus | null {
  ...
  if (
    titleUpper.startsWith("REJECTED") ||
    titleUpper.includes("EVIDENCE REJECTED") ||
    titleUpper.startsWith("अस्वीकृत")
  ) {
    return "rejected";
  }
  if (
    titleUpper.startsWith("CONFIRMED") ||
    titleUpper.includes("EVIDENCE VERIFIED") ||
    titleUpper.startsWith("पुष्टि")
  ) {
    return "confirmed";
  }
  if (
    titleUpper.startsWith("ACTION NEEDED") ||
    titleUpper.includes("MORE EVIDENCE REQUIRED") ||
    titleUpper.startsWith("कार्रवाई")
  ) {
    return "more_info";
  }
  ...
  const isConfirmed =
    ...
    combined.includes("evidence verified") ||
    (combined.includes("confirmed") && !combined.includes("not confirmed"));

  const isMoreInfo =
    combined.includes("more evidence required") ||
    ...
    combined.includes("additional evidence is required");
...
export function pickVetDecisionNotification(notifications: NotificationItem[]): NotificationItem | null {
  const decisionItems = notifications.filter((n) => getVetDecisionStatus(n) !== null);
  return decisionItems[0] ?? notifications.find((n) => n.type === "verification" || n.type === "evidence") ?? null;
}
