...
/** Normalize API payloads that may use snake_case keys or omit array fields. */
export function normalizeEvidenceAnalysis(raw: EvidenceAnalysis | Record<string, unknown>): EvidenceAnalysis {
  const source = raw as Record<string, unknown>;
  const observationsRaw = source.observations;
  const recommendedRaw = source.recommendedActions ?? source.recommended_actions;
  const imageRaw = source.imageAssessment ?? source.image_assessment;

  const recommendedActions = Array.isArray(recommendedRaw)
    ? recommendedRaw.map((item) => {
  ...
  const imageAssessment =
    imageRaw && typeof imageRaw === "object"
      ? {
          validImage: Boolean((imageRaw as Record<string, unknown>).validImage ?? (imageRaw as Record<string, unknown>).valid_image),
          width: Number((imageRaw as Record<string, unknown>).width) || undefined,
          height: Number((imageRaw as Record<string, unknown>).height) || undefined,
          likelyUnrelatedVisual: Boolean(
            (imageRaw as Record<string, unknown>).likelyUnrelatedVisual ??
              (imageRaw as Record<string, unknown>).likely_unrelated_visual
          ),
        }
      : undefined;
  ...
  return {
    ...
    observations: Array.isArray(observationsRaw) ? observationsRaw.map(String) : [],
    recommendedActions,
    analysisMethod: String(source.analysisMethod ?? source.analysis_method ?? "relevance-scoring"),
    disclaimer: String(source.disclaimer ?? ""),
    completeness: (source.completeness as EvidenceAnalysis["completeness"]) ?? undefined,
    relevanceLevel: (source.relevanceLevel ?? source.relevance_level) as EvidenceAnalysis["relevanceLevel"],
    relevanceScore:
      typeof source.relevanceScore === "number"
        ...
        : typeof source.relevance_score === "number"
          ...
          : undefined,
    farmRelated: Boolean(source.farmRelated ?? source.farm_related ?? false),
    imageAssessment,
  };
}
