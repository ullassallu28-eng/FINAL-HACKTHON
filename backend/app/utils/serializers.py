...
from app.schemas.corrective_action import CorrectiveActionResponse, EvidenceAnalysisResponse, SubmittedEvidence
...
def action_to_response(
    ...
    if include_analysis and action.evidence:
        try:
            ...
            raw_analysis = EvidenceAnalysisService.analyze(db, action, action.evidence)
            evidence_analysis = EvidenceAnalysisResponse.model_validate(raw_analysis)
        except Exception:
            # Do not let analysis failures break the list/response for this action.
            evidence_analysis = None
