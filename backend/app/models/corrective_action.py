...
class CorrectiveAction(Base):
    ...
    farm_id: Mapped[str] = mapped_column(String(64), ForeignKey("farms.id"), index=True, nullable=False)
    incident_id: Mapped[str | None] = mapped_column(String(64), ForeignKey("incidents.id"), index=True, nullable=True)
    ...
    status: Mapped[CorrectiveActionStatus] = mapped_column(
        pg_enum(CorrectiveActionStatus), default=CorrectiveActionStatus.PENDING, index=True, nullable=False
    )
    evidence_required: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    verification_status: Mapped[VerificationStatus] = mapped_column(
        pg_enum(VerificationStatus), default=VerificationStatus.UNVERIFIED, index=True, nullable=False
    )
